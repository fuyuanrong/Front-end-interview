...
四、Vue
    1.v-if和v-show的区别？
        都可以控制元素的显示和隐藏
        1.v-show时控制元素的display值来让元素显隐;v-if是通过DOM元素的添加删除来达到显示隐藏
        2.v-if有一个局部编译/卸载的过程,切换中会适当的销毁和重建内部的事件监听和子组件;v-show只是简单的css切换
        3.v-if才是真正的条件渲染;v-show从FALSE变成true的时候不会触发组件的生命周期,v-if会触发生命周期
        4.v-if的切换效率比较低v-show比较高
    2.如何理解MVVM的？
        Model-View-ViewModel的缩写
        Model:模型,对应data数据
        View: 视图,对应用户界面
        ViewModel:视图模型,Vue的实例对象,连接View和Model的桥梁
        核心是提供对View和Model的双向数据绑定,当数据更改时,VM能监听到数据的变化自动更新视图,
        当用户操作视图时,VM也能监听到视图的变化,然后通知数据的更改,实现了双向数据绑定
        VM通过双向数据绑定把View和Model连接起来,他们之间的同步是自动的,不需人为干涉,所以只需关注业务逻辑即可,不需操作DOM,同时也不需关注数据的状态问题,因为它是统一由MVVM管理
    3.v-for中的key值的作用是什么？
        key 是DOM元素的唯一标识
        作用:
            1.提高虚拟DOM更新效率
            2.若不设置key 可能会触发一下bug
            3.为了触发过渡效果
    4.说一下你对vue生命周期的理解。
        before
        ...
        在使用了keep-alive后 多了activited  deactivited
    5.在created和mounted去请求数据，有什么区别？
        created:在渲染前调用,通常先初始化属性,然后再渲染
        mounted:在模板渲染后,一般都是初始化页面后,在对元素节点进行操作,在这里请求数据可能会出现闪屏问题
        请求的数据对DOM有影响就使用created,没有就放在mounted
    6.vue中的修饰符有哪些？
    ...
    9.keep-alive是什么？怎么使用？
        vue的内置组件,包裹组件时会缓存不活跃的组件实例,并不是销毁
    ...
    11.vue路由时怎么传参的？
        ...
        1.params传参
            ...
            路由属性传参:
            this.$router.push({name:'/index/${item.id}/${item.code}'})
            路由配置:{path:'/index/:id/:code'}
            params是路由的一部分,必须要在路由后面添加参数名。
            params一旦设置在路由，params就是路由的一部分，如果这个路由有params传参，但是在跳转的时候没有传这个参数，会导致跳转失败或者页面会没有内容
        2.query传参(可避免页面刷新参数丢失的问题)
            path和name传参跳转都可以
            this.$router.push({
                name:'index',
                query:{
    ...
    12.vue路由的hash模式和history模式有什么区别？
        ...
        2.再做回车刷新的时候,hash模式会加载对应页面,history会报错
        3.hash模式支持低版本浏览器,history不支持,是因为H5新增的API
    ...
    16.vuex刷新数据会丢失吗？怎么解决？
        vuex会重新获取数据,页面会丢失数据
            1.把数据直接保存到浏览器缓存里(cookie sessionstorage localstorage)
            2.页面刷新时再次请求数据,达到动态更新的方法
                监听浏览器的刷新时间,在刷新前把数据保存到sessionstorage里,刷新后请求数据,请求到了用vuex,没有就用sessionstorage内的
    17.computed和watch的区别？
        1.computed时计算属性,watch是监听属性,监听的是data中数据的变化
        2.computed支持缓存,依赖属性值发生变化,计算属性才会重新计算,否则不用缓存;watch不支持缓存
        3.computed不支持异步,watch支持异步操作
        4.computed是第一次页面加载就监听,watch不会
        5.computed函数中必须要return watch不要
    18.vuex在什么场景会去使用？属性有哪些？
        ...
        使用场景:
            用户个人信息,购物车模块,订单模块
    19.vue的双向数据绑定原理是什么？
        通过数据劫持和发布订阅者模式来实现,同时利用Object.defineProperty()来劫持各个属性的getter和setter
        在数据发生改变时发送消息给订阅者,触发对应的监听回调渲染视图,也就是说数据和视图是同步的,数据发生改变视图也跟着改变,视图发生改变,数据也跟着改变
    20.了解diff算法和虚拟DOM吗？
        虚拟DOM:描述元素和元素之间的关系,创建一个js对象
            如果组件有响应的数据,并发生改变,render函数会生成一个新的虚拟DOM,新的虚拟DOM回合旧的进行比对,找到需要修改的虚拟DOM,然后去对应的真实DOM中修改
        Diff算法:虚拟DOM比对时用的,返回一个patch对象,这个对象的作用就是存储比较新旧两个虚拟DOM不同的地方,最后用patch里返回记录的信息进行更新真实DOM
        步骤:
        1.JS对象表示真实的DOM结构,要生成一个虚拟DOM,再用虚拟DOM构建一个真实DOM,渲染页面
        2.状态发生改变生成新的虚拟DOM,跟旧的虚拟DOM进行比对,这个比对过程就是Diff算法,利用patch记录差异
        3.把记录的差异用在第一份虚拟DOM生成真实DOM上,视图就更新了

    21.vue和jquery的区别是什么？
        1.原理不同
            vue是数据绑定:jq是先获取dom在处理
        2.着重点不同
            vue是数据驱动;jq是着重于页面
        3.操作不同
        4.未来发展不同
    22.vuex的响应式处理。
        vuex是vue的状态管理工具
        vue中可以直接触发methods中的方法,vuex不可以的,为了处理异步,当触发事件的时候,会通过dispatch来访问cation中的方法
        action中的commit会触发mutation中的方法从而修改state里的值,通过getter把数据更新到视图
    ...
    29.vue3和vue2有哪些区别？
        1.双向数据绑定原理不同  Object.defineProperty  Proxy
        2.是否支持碎片  fragment vue2只能有一个根节点 vue3不是
        3.API不同;选项式api和组合式api
        4.定义数据变量方法不同  vue2数据放在data()里  vue3使用setup 使用reactive  ref分别定义复杂简单数据类型
        5.生命周期不同 
        6.传值不同  
        7.指令和插槽不同 
        8.main.js不同
    30.vue的性能优化怎么做？
        1.编码优化
            不要把所有数据都放在data中
            v-for是给每个元素绑定事件用事件代理
            keep-alive缓存组件
            尽可能拆分组件,提高复用性,维护性
            key值要保证唯一
            合理使用路由懒加载
            数据如果持久化使用存储尽量用防抖,节流优化
        2.加载优化
            按需加载
            内容懒加载
            图片懒加载
    ...
    33.vue3为什么使用proxy？
        ...
        JS中做属性拦截常见的方法有三:defineProperty getter/setter和Proxies
        Vue2中使用 defineProperty 的原因是,当时2013年只能用这种方式。
        由于该API存在一些局限性,比如对数组的拦截有问题,为此vue需要专门为数组响应式做一套实现。另外不能拦截那些新增/删除属性;最后defineProperty 方案在初始化时需要深度递归遍历待处理对象才能对他完全拦截,明显增加了初始化的时间
        以上两点在Proxy出现后迎刃而解,不仅可以对数组实现拦截,还能对Map Set实现拦截;另外Proxy的拦截也是懒处理行为,如果用户没有访问嵌套对象,那么也不会实施拦截,这就让初始化的速度和内存占用都改善了
        当然Proxy是由兼容性的问题,IE完全不支持,所以如果需要IE支持就不合适
        注:Proxy产生于ES2015

        Proxy属性拦截的原理:利用get set deleteProperty 这三个trap实现拦截
        ...
        Object.defineProperty属性拦截原理:利用get set 这两个trap实现拦截
    ...
    36.是否使用过nuxt.js？
        ...
        SSR:服务端渲染
            好处:SSR生成的是有内容的HTML页面,有利于搜索引擎的搜索
                优化首屏加载事件
            SEO:优化搜索引擎
            SPA单页面应用不利于搜索引擎seo搜索操作
    37.SEO如何优化？
        1.SSR服务端渲染
...
七、Weabpack
    ...
    2.webpack是怎么打包的，babel是做什么的？
        webpack会把js,css,img看做一个模块,用import/require引入
        找到入口文件,通过入口文件找到关联的依赖文件,把他们打包在一起
        把bundle文件,拆分成多个小的文件,异步按需加载所需要的文件
        如果一个文件被多个地方引用,打包只会生成一个文件,
        如果没有被引用,不会打包,如果引入的变量和方法没有调用也不会打包
        对于多个入口文件,加入相同的代码,可以用插件把他抽离到公共文件中
